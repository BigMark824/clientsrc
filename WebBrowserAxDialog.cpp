	...
	try{
		...
		if (!siteSEO)
		{
			std::string requestTmp(
				...
				"" + videoSEOInfo + "\r\n"
				"For more games visit http://www.quaver.lol\r\n"
				"</media:description>\r\n"
				"<media:category\r\n"
...
void WebBrowserAxDialog::DoUploadVideo(std::string token, std::string title, std::string seostr, int placeId)
{
	...
	if (seostr.empty())
	{
		...
		if (placeId > 0) 
		{
			videoSEOInfo = format_string("To play this game, please visit: http://www.quaver.lol/item.aspx?id=%d&amp;rbx_source=youtube&amp;rbx_medium=uservideo", placeId);
		} else {
			videoSEOInfo = seostr;
		}
	}
